...
(defun dotspacemacs/user-config ()
  ...
  ;; Signals
  (defvar	SIGHUP 1 "制御端末(controlling terminal)のハングアップ検出、または制御しているプロセスの死")
  (defvar	SIGINT 2 "キーボードからの割り込み (Interrupt)")
  (defvar	SIGQUIT 3 "キーーボードによる中止 (Quit)")
  (defvar SIGILL 4 "不正な命令")
  (defvar SIGTRAP 5 "トレース/ブレークポイント トラップ, IOT トラップ。SIGABRT と同義")
  (defvar SIGABRT 6 "abort(3) からの中断 (Abort) シグナル")
  (defvar SIGBUS 7 "バスエラー (不正なメモリアクセス)")
  (defvar SIGFPE 8 "浮動小数点例外")
  (defvar SIGKILL 9 "Kill シグナル")
  (defvar SIGUSR1 10 "ユーザ定義シグナル 1")
  (defvar SIGSEGV 11 "不正なメモリ参照")
  (defvar SIGUSR2 12 "ユーザ定義シグナル 2")
  (defvar SIGPIPE 13 "パイプ破壊:読み手の無いパイプへの書き出し")
  (defvar SIGALRM 14 "alarm(2) からのタイマーシグナル")
  (defvar SIGTERM 15 "終了 (termination) シグナル")
  (defvar SIGSTKFLT 16 "数値演算プロセッサにおけるスタックフォルト (未使用)")
  (defvar SIGCHLD 17 "子プロセスの一時停止 (stop) または終了")
  (defvar SIGCONT 18 "一時停止 (stop) からの再開")
  (defvar SIGSTOP 19 "プロセスの一時停止 (stop)")
  (defvar SIGTSTP 20 "端末より入力された一時停止 (stop)")
  (defvar SIGTTIN 21 "バックグランドプロセスの端末入力")
  (defvar SIGTTOU 22 "バックグランドプロセスの端末出力")
  (defvar SIGURG 23 "ソケットの緊急事態 (urgent condition) (4.2BSD)")
  (defvar SIGXCPU 24 "CPU時間制限超過 (4.2BSD)")
  (defvar SIGXFSZ 25 "ファイルサイズ制限の超過 (4.2BSD)")
  (defvar SIGVTALRM 26 "仮想アラームクロック (4.2BSD)")
  (defvar SIGPROF 27 "profiling タイマの時間切れ")
  (defvar SIGWINCH 28 "ウィンドウ リサイズ シグナル (4.3BSD, Sun)")
  (defvar SIGIO 29 "入出力が可能になった (4.2BSD)")
  (defvar SIGPWR 30 "電源喪失 (Power failure) (System V)")
  (defvar SIGSYS 31 "ルーチンへの引き数が不正 (SVr4)")

  ;; Linux realtime signals
  (defvar SIGRTMIN 34)
  (defvar SIGRTMIN+1 35)
  (defvar SIGRTMIN+2 36)
  (defvar SIGRTMIN+3 37)
  (defvar SIGRTMIN+4 38)
  (defvar SIGRTMIN+5 39)
  (defvar SIGRTMIN+6 40)
  (defvar SIGRTMIN+7 41)
  (defvar SIGRTMIN+8 42)
  (defvar SIGRTMIN+9 43)
  (defvar SIGRTMIN+10 44)
  (defvar SIGRTMIN+11 45)
  (defvar SIGRTMIN+12 46)
  (defvar SIGRTMIN+13 47)
  (defvar SIGRTMIN+14 48)
  (defvar SIGRTMIN+15 49)
  (defvar SIGRTMAX-14 50)
  (defvar SIGRTMAX-13 51)
  (defvar SIGRTMAX-12 52)
  (defvar SIGRTMAX-11 53)
  (defvar SIGRTMAX-10 54)
  (defvar SIGRTMAX-9 55)
  (defvar SIGRTMAX-8 56)
  (defvar SIGRTMAX-7 57)
  (defvar SIGRTMAX-6 58)
  (defvar SIGRTMAX-5 59)
  (defvar SIGRTMAX-4 60)
  (defvar SIGRTMAX-3 61)
  (defvar SIGRTMAX-2 62)
  (defvar SIGRTMAX-1 63)
  (defvar SIGRTMAX 64)

  (defun sig--send-signal (signal-number &optional proc)
    (let ((proc_ (or proc (get-buffer-process (current-buffer)))))
      (if proc_
          (progn
            (signal-process proc_ signal-number)
            t)
        nil)))

  (defun sig-hup (&optional proc) (interactive) (sig--send-signal SIGHUP proc))
  (defun sig-int (&optional proc) (interactive) (sig--send-signal SIGINT proc))
  (defun sig-quit (&optional proc) (interactive) (sig--send-signal SIGQUIT proc))
  (defun sig-ill (&optional proc) (interactive) (sig--send-signal SIGILL proc))
  (defun sig-trap (&optional proc) (interactive) (sig--send-signal SIGTRAP proc))
  (defun sig-abrt (&optional proc) (interactive) (sig--send-signal SIGABRT proc))
  (defun sig-bus (&optional proc) (interactive) (sig--send-signal SIGBUS proc))
  (defun sig-fpe (&optional proc) (interactive) (sig--send-signal SIGFPE proc))
  (defun sig-kill (&optional proc) (interactive) (sig--send-signal SIGKILL proc))
  (defun sig-usr1 (&optional proc) (interactive) (sig--send-signal SIGUSR1 proc))
  (defun sig-segv (&optional proc) (interactive) (sig--send-signal SIGSEGV proc))
  (defun sig-usr2 (&optional proc) (interactive) (sig--send-signal SIGUSR2 proc))
  (defun sig-pipe (&optional proc) (interactive) (sig--send-signal SIGPIPE proc))
  (defun sig-alrm (&optional proc) (interactive) (sig--send-signal SIGALRM proc))
  (defun sig-term (&optional proc) (interactive) (sig--send-signal SIGTERM proc))
  (defun sig-stkflt (&optional proc) (interactive) (sig--send-signal SIGSTKFLT proc))
  (defun sig-chld (&optional proc) (interactive) (sig--send-signal SIGCHLD proc))
  (defun sig-cont (&optional proc) (interactive) (sig--send-signal SIGCONT proc))
  (defun sig-stop (&optional proc) (interactive) (sig--send-signal SIGSTOP proc))
  (defun sig-tstp (&optional proc) (interactive) (sig--send-signal SIGTSTP proc))
  (defun sig-ttin (&optional proc) (interactive) (sig--send-signal SIGTTIN proc))
  (defun sig-ttou (&optional proc) (interactive) (sig--send-signal SIGTTOU proc))
  (defun sig-urg (&optional proc) (interactive) (sig--send-signal SIGURG proc))
  (defun sig-xcpu (&optional proc) (interactive) (sig--send-signal SIGXCPU proc))
  (defun sig-xfsz (&optional proc) (interactive) (sig--send-signal SIGXFSZ proc))
  (defun sig-vtalrm (&optional proc) (interactive) (sig--send-signal SIGVTALRM proc))
  (defun sig-prof (&optional proc) (interactive) (sig--send-signal SIGPROF proc))
  (defun sig-winch (&optional proc) (interactive) (sig--send-signal SIGWINCH proc))
  (defun sig-io (&optional proc) (interactive) (sig--send-signal SIGIO proc))
  (defun sig-pwr (&optional proc) (interactive) (sig--send-signal SIGPWR proc))
  (defun sig-sys (&optional proc) (interactive) (sig--send-signal SIGSYS proc))
...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (ghub go-guru go-eldoc company-go go-mode flycheck-pyflakes flymake-cursor flymake-python-pyflakes flymake-easy jedi jedi-core python-environment epc ctable concurrent deferred ob-elixir flycheck-mix flycheck-credo alchemist elixir-mode python-x folding multi-eshell tide typescript-mode flycheck csv-mode goto-chg projectile helm helm-core async review-mode twittering-mode rvm ruby-tools ruby-test-mode rubocop rspec-mode robe rbenv rake minitest chruby bundler inf-ruby web-mode tagedit slim-mode scss-mode sass-mode pug-mode less-css-mode helm-css-scss haml-mode emmet-mode company-web web-completion-data helm-gtags ggtags atomic-chrome websocket plantuml-mode yaml-mode switch-buffer-functions elnode db fakir creole web noflet kv restclient-helm ob-restclient ob-http company-restclient know-your-http-well restclient sql-indent wakatime-mode org-projectile org-category-capture org-present org-pomodoro alert log4e gntp org-download htmlize gnuplot web-beautify livid-mode helm-company helm-c-yasnippet fuzzy company-tern dash-functional tern company-statistics company-anaconda company auto-yasnippet ac-ispell auto-complete skewer-mode simple-httpd json-mode json-snatcher json-reformat js2-refactor yasnippet multiple-cursors js2-mode js-doc coffee-mode yapfify pyvenv pytest pyenv-mode py-isort pip-requirements live-py-mode hy-mode helm-pydoc cython-mode anaconda-mode pythonic helm-elscreen elscreen-multi-term elscreen helm-mt multi-term mmm-mode markdown-toc markdown-mode gh-md magit magit-popup git-commit with-editor ws-butler winum volatile-highlights vi-tilde-fringe uuidgen toc-org spaceline powerline restart-emacs request rainbow-delimiters popwin persp-mode pcre2el paradox spinner org-plus-contrib org-bullets open-junk-file neotree move-text lorem-ipsum linum-relative link-hint indent-guide hungry-delete hl-todo highlight-parentheses highlight-numbers parent-mode highlight-indentation hide-comnt help-fns+ google-translate golden-ratio flx-ido fill-column-indicator fancy-battery eyebrowse expand-region exec-path-from-shell evil-visualstar evil-visual-mark-mode evil-unimpaired evil-tutor evil-surround evil-search-highlight-persist evil-numbers evil-nerd-commenter evil-mc evil-matchit evil-lisp-state smartparens evil-indent-plus evil-iedit-state iedit evil-exchange evil-escape evil-ediff evil-args evil-anzu anzu eval-sexp-fu highlight dumb-jump f dash s define-word column-enforce-mode clean-aindent-mode auto-highlight-symbol aggressive-indent adaptive-wrap ace-window ace-link which-key use-package macrostep hydra helm-themes helm-swoop helm-projectile helm-mode-manager helm-make helm-flx helm-descbinds helm-ag evil elisp-slime-nav bind-map auto-compile ace-jump-helm-line))))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
